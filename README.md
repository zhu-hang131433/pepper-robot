...
| `--silence-seconds` | `0.45` | 用户停顿多久后判定说完；为降低短问句延迟而缩短；若常截断句子可增至 `0.6`～`0.8` |
  | `--start-threshold` | `420` | 开始说话的音量阈值；现场很吵时适当调高 |
| `--silence-threshold` | `400` | 高于此音量视为仍在说话；现场底噪较大时应提高该值 |
| `--start-timeout` | `6` | Pepper 提示后等待用户开始说话的秒数 |
| `--max-seconds` | `15` | 单个问题允许的最长录音时长 |

默认起声阈值已调为 `420`，用于减少环境声误触发；默认静音判定为 `0.45` 秒，以缩短短问句的提交等待。识别结果为空或只有一个字符时会直接忽略，不会调用百炼或让 Pepper 回复。若发现用户在一句话中的短暂停顿被误判为结束，再将 `--silence-seconds` 调回 `0.6`。
